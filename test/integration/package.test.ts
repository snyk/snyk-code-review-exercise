...
jest.setTimeout(10000); // Set the timeout to 12 seconds
...
describe('/package/:name/:version endpoint', () => {
  ...
  it('returns 404 for non-existent packages', async () => {
    const packageName = 'nonexistent-package';
    const packageVersion = '1.0.0';

    await expect(
      got(`http://localhost:${port}/package/${packageName}/${packageVersion}`)
    ).rejects.toThrow(/404/);
  });

  it('returns 400 for non-existent versions', async () => {
    const packageName = 'react';
    const packageVersion = '300.13.0';
